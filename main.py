...
def parse_booking_message(body: str) -> Dict[str, Any]:
    ...
    # Remove obvious 4-digit years so they don't pollute the time parsing
    cleaned_no_years = YEAR_REGEX.sub(" ", cleaned)

    # Try to isolate the date/time phrase first
    dt_source = None
    ...
    if dt_source:
        try:
            ...
            # If no explicit minutes in the text (e.g. "8pm"), force :00
            if re.search(r"\b\d{1,2}\s*(am|pm)\b", dt_source, flags=re.IGNORECASE) \
               and not re.search(r":\d{2}", dt_source):
