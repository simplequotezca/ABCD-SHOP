...
def log_estimate_lead(shop: ShopConfig, from_number: str, result: Dict[str, Any]):
    ...
    # Prefer embedded data URLs (viewable without Imgur/Twilio auth)
    image_sources: List[str] = (
        result.get("image_data_urls")
...
def log_booking_event(shop: ShopConfig, from_number: str,
    ...
    if last_estimate:
        ...
        image_sources = (
            last_estimate.get("image_data_urls")
...
async def sms_webhook(request: Request, token: str):
    ...
    try:
        ...
        # Normalized quick-reply body for 1/2 handling
        ...
        # --------------------------------------------------------
        if num_media == 0:
            # Handle quick replies 1 / 2 more robustly
        ...
        # Build embedded data URLs so shops can view photos directly in Calendar
        ...
        # Attach image sources so calendar events can embed photos
        ...
        # DEMO-LOCKED: Never let an exception crash Twilio response
