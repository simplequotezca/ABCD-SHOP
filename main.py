...
def safe_json_parse(raw: str) -> Any:
    """
    Strip code fences / 'json' labels and parse as JSON.
    """
    text = raw.strip()
    if text.startswith("```"):
        parts = text.split("```")
...
def analyze_damage(image_url: str, shop: ShopConfig) -> Dict[str, Any]:
    """
    Ultra-detailed A2 estimator using GPT-4.1 with vision.
    Returns a rich JSON object with:
      - damage_summary, areas, damage_types, severity
      - recommended_labor_hours, estimated_cost_min, estimated_cost_max
      - parts_required[], structural_concerns[], paint_operations[]
      - recommended_actions[], notes, confidence_score
    """
    if not can_use_ai():
    ...
    system_prompt = (
        ...
        "Always describe vehicle orientation from the DRIVER'S point of view:\n"
        "- driver side = left\n"
        "- passenger side = right\n\n"
        "You estimate labor and costs for collision damage using the shop's pricing data."
    )

    user_prompt = f"""
Look at the vehicle damage in the photo and produce a single STRICT JSON object.

Shop pricing (JSON):
{pricing}

Your job:
- Estimate visible collision damage based ONLY on what is clearly visible in the image.
- Use realistic labor hours and cost ranges for Ontario, Canada.

Output a single JSON object with EXACTLY these keys:

- "damage_summary": string
    A clear paragraph summarizing what is damaged and how, from the driver's point of view.

- "areas": string[]
    List of major body areas affected (e.g. "front bumper", "hood", "left headlight", "right rear door").

- "damage_types": string[]
    Types of damage (e.g. "scratches", "deep dent", "cracked", "panel misalignment", "paint scuff").

- "severity": string
    One of: "minor", "moderate", "severe".

- "recommended_labor_hours": number
    Total hours for body + paint + related work as a single number.

- "estimated_cost_min": number
- "estimated_cost_max": number
    Estimated cost range in CAD for this shop, consistent with the pricing provided.

- "parts_required": [
    {{
      "part": string,        // e.g. "front bumper cover", "left headlight assembly"
      "operation": string    // one of "repair", "replace", "inspect"
    }}
  ]

- "structural_concerns": string[]
    Possible hidden or structural/frame concerns to inspect. Can be empty if none.

- "paint_operations": string[]
    Paint steps needed (e.g. "prep", "prime", "basecoat", "clearcoat", "blend adjacent panel").

- "recommended_actions": string[]
    High-level recommendations for the shop (e.g. "Remove and replace front bumper cover", "Perform 4-wheel alignment check").

- "notes": string
    Any caveats, assumptions, or things the shop should double-check in person.

- "confidence_score": number
    A float from 0.0 to 1.0 indicating your confidence in this estimate.

Rules:
- Output ONLY valid JSON. No markdown, no commentary, no explanation.
- Ensure all keys above are present.
- Severity and cost range must align with visible damage and the pricing provided.
"""
...
def parse_booking(user_text: str, analysis: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    """
    Turn free-text booking message into structured JSON with:
      - name, phone, email, datetime_iso (America/Toronto), notes
    """
    if not can_use_ai():
        return None
    ...
    system_prompt = (
        "You are an assistant that converts free-form customer SMS messages into structured booking data "
        "for an auto body shop. Output MUST be valid JSON only."
    )

    user_prompt = f"""
Convert the following customer message into booking info.

Customer message:
\"\"\"{user_text}\"\"\"

Latest AI damage summary (may be empty):
\"\"\"{summary}\"\"\"

Assume the shop is in the 'America/Toronto' timezone.

Return a single JSON object with exactly these keys:
- "name": string
- "phone": string
- "email": string
- "datetime_iso": string   // ISO 8601, e.g. "2025-11-30T14:30:00"
- "notes": string          // any extra info from the message

Rules:
- If you cannot confidently infer a specific appointment date/time, set "datetime_iso" to "" (empty string).
...
"""
    ...
    try:
        ...
        if not data.get("datetime_iso"):
            # If no valid datetime, treat as failure so we prompt user again.
            return None
        return data
    except Exception as e:
...
# ============================================================
# TEXT BUILDERS (ULTRA-DETAILED BUT CLEAN)
# ============================================================
...
def format_damage_text(shop: ShopConfig, a: Dict[str, Any]) -> str:
    """
    First detailed message after AI analysis – used to impress customers & shops.
    """
...
def format_full_estimate_text(shop: ShopConfig, a: Dict[str, Any]) -> str:
    """
    Second message – full estimate breakdown with cost + labor.
    """
    eid = str(uuid.uuid4())
    ...
    structural_line = (
        "Structural concerns: " + (", ".join(structural_concerns) if structural_concerns else "None specifically noted.")
    )
    paint_line = (
    ...
    return (
        ...
        f"Estimated labor: {hours} hours (body + paint)\n\n"
        f"{structural_line}\n"
        f"{paint_line}\n\n"
        "This is a visual, preliminary estimate based on photos only. Final pricing may change after an in-person inspection.\n\n"
        "To book an appointment, reply with your name, phone number, email, "
        "and preferred date/time (e.g. \"John Doe, 416-555-1234, john@email.com, "
        "Tuesday at 3pm\")."
    )
...
def bg_process_image(shop_token: str, shop: ShopConfig, from_phone: str, media_url: str):
    """
    Runs in the background after image is received.
    """
    try:
        if not can_use_ai():
        ...
        # Send ultra-detailed but clean analysis text
...
def bg_process_booking(shop: ShopConfig, from_phone: str, text: str, last_analysis: Optional[Dict[str, Any]]):
    """
    Runs in the background when customer sends booking info.
    """
    try:
        ...
        if not booking:
            send_sms(
                from_phone,
                "⚠️ I couldn't understand your booking details.\n"
                "Please resend your name, phone number, email, and preferred date/time."
            )
            return
        ...
        send_sms(
            from_phone,
            "✅ Your appointment request has been received and added to the shop’s calendar.\n"
        ...
        send_sms(
            from_phone,
            "⚠️ There was a problem booking your appointment. Please try again or call the shop directly."
        )
...
async def sms_webhook(request: Request, background_tasks: BackgroundTasks):
    ...
    if media_url:
        ...
        msg.message(
            ...
            "Our AI is analyzing the damage now – you'll get your detailed estimate by text shortly."
        )
        return PlainTextResponse(str(msg), media_type="application/xml")
    ...
    if lower == "2":
        msg = MessagingResponse()
        msg.message("No problem! Please send a few more photos of the damage from different angles.")
        return PlainTextResponse(str(msg), media_type="application/xml")
