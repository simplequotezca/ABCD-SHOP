...
VERSION = "demo_locked_main25_v2_2"  # upgraded with leads + gallery + email
...
# Base URL for building public links (gallery, images)
APP_BASE_URL = os.getenv("APP_BASE_URL")
if not APP_BASE_URL:
    raise RuntimeError(
        "APP_BASE_URL is required (e.g. 'https://your-app.up.railway.app')"
    )

# Local folder for saving customer photos
PHOTOS_DIR = "photos"
...
class Lead(Base):
    ...
    damaged_areas = Column(Text, nullable=True)    # comma-separated
    damage_types = Column(Text, nullable=True)     # comma-separated
...
if not DATABASE_URL:
    print("WARNING: DATABASE_URL not set – DB lead storage disabled.")
    engine = None
    SessionLocal = None
else:
    try:
...
# ============================================================
# Email notification config (for shop booking alerts)
# ============================================================
...
# ============================================================
# Helpers: Twilio image download + local save + gallery link
# ============================================================
...
def save_customer_photo(image_bytes: bytes) -> str:
    """
    Save image bytes to /photos and return a public URL
    served via /images/{photo_id}.jpg
    """
...
def build_gallery_url(image_urls: List[str]) -> Optional[str]:
    """
    Build a compact, shareable gallery URL that decodes back
    into the list of hosted image URLs.
    """
    if not image_urls:
        return None
...
async def view_photos(payload: str):
    """
    Clean gallery page for shops to view all photos for a lead.
    """
    try:
        decoded = base64.urlsafe_b64decode(payload.encode()).decode()
        urls = json.loads(decoded)
        if not isinstance(urls, list):
            raise ValueError("Decoded payload is not a list")
    except Exception:
    ...
    cards_html = "".join(
        ...
        <div style="background:#ffffff;border-radius:12px;
            ...
            <img src="{u}" style="width:100%;border-radius:10px;display:block;">
        </div>
        """
        for u in urls
    )
    ...
    <html>
        ...
        <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Arial, sans-serif;
            ...
            <div style="max-width: 720px; margin: 0 auto;">
                <h2 style="margin-bottom: 16px; color:#111827;">Customer Photos</h2>
                <p style="margin-bottom: 24px; color:#4b5563;">
                    Sent via AI Estimator. Use these images together with the AI summary and your
                    in-person inspection to prepare an accurate repair plan.
                </p>
                {cards_html or "<p>No photos available.</p>"}
            </div>
        </body>
    </html>
    """
...
def mark_leads_booked(shop: ShopConfig, phone: str):
    ...
    try:
        leads = (
            session.query(Lead)
            .filter(Lead.shop_id == shop.id, Lead.phone == phone)
            .all()
        )
        for l in leads:
            l.appointment_booked = True
        session.commit()
    except Exception as e:
        print("DB mark_leads_booked error:", e)
        session.rollback()
    finally:
        session.close()


def send_booking_email(shop: ShopConfig, description: str):
    """
    Email the shop when a booking is created.
    Uses SMTP_* env vars + SHOP_NOTIFICATION_EMAIL.
    If not configured, silently skips.
    """
    ...
    try:
        msg = EmailMessage()
        msg["Subject"] = f"New AI Booking – {shop.name}"
        msg["From"] = EMAIL_FROM
        msg["To"] = SHOP_NOTIFICATION_EMAIL

        body = (
            f"A new booking was created for {shop.name}.\n\n"
            f"{description}\n"
        )
        ...
        print("Booking email sent to", SHOP_NOTIFICATION_EMAIL)
    except Exception as e:
        print("Booking email send error:", e)

# ============================================================
# Lead + booking logging (Calendar + DB + Email)
# ============================================================
...
def log_estimate_lead(shop: ShopConfig, from_number: str, result: Dict[str, Any]):
    ...
    areas_list = result.get("damaged_areas", []) or []
    types_list = result.get("damage_types", []) or []
    summary = result.get("summary", "")

    areas = ", ".join(areas_list) or "N/A"
    types = ", ".join(types_list) or "N/A"

    # Prefer hosted image URLs (served from this app) for gallery
    ...
    gallery_url = build_gallery_url(image_sources) if image_sources else None

    title = f"Lead (Estimate only) – {shop.name}"

    lines = [
        "Lead generated by AI Estimator",
        "",
        f"Customer Phone: {from_number}",
        "",
        "AI Estimate (preliminary):",
        f"• Severity: {severity}",
        f"• Estimated Cost Range: ${est_min} – ${est_max}",
        ...
        summary,
        "",
        "Status: LEAD – NO APPOINTMENT BOOKED",
        f"Captured: {datetime.now(LOCAL_TZ).strftime('%Y-%m-%d %I:%M %p %Z')}",
    ]

    if gallery_url:
        lines.extend(["", "Customer Photos:", gallery_url])

    description = "\n".join(lines)

    create_calendar_event(shop, title, description)

    # Persist lead to DB so shops can see all inquiries, even without bookings
    save_lead_to_db(
        shop=shop,
        phone=from_number,
        severity=severity,
        ...
        est_max=est_max,
        summary=summary,
        damaged_areas=areas_list,
        damage_types=types_list,
...
def log_booking_event(
    ...
    if last_estimate:
        areas_list = last_estimate.get("damaged_areas", []) or []
        types_list = last_estimate.get("damage_types", []) or []

        lines.extend(
            [
                "",
                "AI Estimate (linked to this booking):",
                f"• Severity: {last_estimate.get('severity')}",
                f"• Estimated Cost Range: ${last_estimate.get('estimated_cost_min')} – "
                f"${last_estimate.get('estimated_cost_max')}",
                f"• Damaged Areas: {', '.join(areas_list) or 'N/A'}",
                f"• Damage Types: {', '.join(types_list) or 'N/A'}",
                "",
                "Summary:",
        ...
        image_sources: List[str] = (
            last_estimate.get("hosted_image_urls")
            or last_estimate.get("image_urls")
            or last_estimate.get("image_data_urls")
            or []
        )
        if image_sources:
            gallery_url = build_gallery_url(image_sources)
            lines.extend(["", "Customer Photos:", gallery_url])
    ...
    # Mark all leads for this phone as having booked
    mark_leads_booked(shop, phone)

    # Notify shop via email (if configured)
...
async def sms_webhook(request: Request, token: str):
    try:
        ...
        if num_media == 0:
            ...
            # Handle quick replies FIRST so nothing else fires
        ...
        for i in range(num_media):
            ...
            try:
                ...
                # Save locally and build hosted URL for gallery / calendar
                hosted_url = save_customer_photo(img)
        ...
        # Attach all image reference types for downstream usage
        result["image_urls"] = image_urls
        result["image_data_urls"] = embedded_data_urls
        result["hosted_image_urls"] = hosted_image_urls
