...
async def view_photos(payload: str):
    """
    Simple gallery page for shops to view all photos for a lead.
    """
    try:
    ...
    img_html = "".join(
        f'<div><img src="{u}" '
        f'style="width:100%;max-width:600px;margin-bottom:20px;border-radius:8px;"></div>'
        for u in urls
    )
    ...
    <html>
        ...
        <body style="font-family: Arial, sans-serif; padding: 20px; background-color: #f5f5f5;">
            <h2 style="margin-bottom: 20px;">Customer Photos</h2>
            {img_html or "<p>No photos available.</p>"}
        </body>
    </html>
    """
...
def log_estimate_lead(shop: ShopConfig, from_number: str, result: Dict[str, Any]):
    ...
    description = (
        ...
        f"Summary:\n{result.get('summary')}\n\n"
        f"Status: LEAD – NO APPOINTMENT BOOKED\n"
        f"Created at: {datetime.now(LOCAL_TZ).strftime('%Y-%m-%d %H:%M %Z')}"
    )
