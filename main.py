...
def severity_plan(severity: str) -> Tuple[str, List[str], List[str]]:
    ...
    NOTE: Hours are derived from the operations list (real estimator style),
    not hard-coded per severity.
    """
    if severity == "Minor":
...
# Operation-based labor hour ranges (min, max)
OP_HOURS: Dict[str, Tuple[float, float]] = {
    # Bumper
    "Repair bumper": (1.0, 2.0),
    "Refinish bumper": (1.0, 2.0),
    "Replace bumper": (2.0, 3.0),

    # Fender
    "Repair fender": (2.0, 4.0),
    "Replace fender": (3.0, 4.0),

    # Lighting
    "Replace headlight": (1.0, 2.0),

    # Hood
    "Repair hood": (5.0, 8.0),
    "Replace hood": (3.0, 5.0),

    # Suspension / mechanical check
    "Inspect suspension": (3.0, 5.0),
}

# Severity overhead covers teardown, scan/diag, fitment, clips, alignment checks, etc.
...
def hours_from_operations(severity: str, operations: List[str]) -> Tuple[int, int]:
    """
    Sums operation-based hour ranges + severity overhead.
    Returns integer (min_hours, max_hours).
    """
    min_h = 0.0
    max_h = 0.0
    ...
    for op in operations:
        ...
        if rng:
            ...
            # Unknown ops still cost time; keep conservative.
            min_h += 1.0
            max_h += 2.0

    oh = SEVERITY_OVERHEAD.get(severity, (2.0, 4.0))
    min_h += oh[0]
    max_h += oh[1]

    min_i = max(1, int(round(min_h)))
    max_i = max(min_i, int(round(max_h)))
    return min_i, max_i
...
async def estimate_api(
    ...
    hours_min, hours_max = hours_from_operations(severity, operations)
