...
async def root_redirect():
    # Default to the first shop demo
    return Response(status_code=307, headers={'Location':'/quote?shop_id=miss'})
...
# ============================================================
# GOOGLE CALENDAR – REAL (SERVICE ACCOUNT)
# ============================================================
...
def _load_service_account_info() -> Dict[str, Any]:
    if GOOGLE_SA_JSON_B64:
        try:
            raw = base64.b64decode(GOOGLE_SA_JSON_B64).decode("utf-8")
            return json.loads(raw)
        except Exception as e:
            raise RuntimeError(f"Bad GOOGLE_SERVICE_ACCOUNT_JSON_B64: {e}")
    if GOOGLE_SA_JSON:
        try:
            return json.loads(GOOGLE_SA_JSON)
        except Exception as e:
            raise RuntimeError(f"Bad GOOGLE_SERVICE_ACCOUNT_JSON: {e}")
    raise RuntimeError("Missing GOOGLE_SERVICE_ACCOUNT_JSON (or _B64).")
...
def _calendar_service():
    ...
    # cache_discovery=False avoids writing cache files on ephemeral hosts
...
def create_calendar_event(shop: ShopConfig, booking: BookingRequest) -> str:
    """Creates the calendar event and returns the eventId."""
    svc = _calendar_service()

    # Ensure timezone awareness. Railway/servers are UTC; you can change SHOP_TZ later.
    tz = os.getenv("SHOP_TIMEZONE", "America/Toronto")

    event = {
        "summary": f"New booking — {booking.customer_name} ({booking.vehicle})",
        "description": (
            f"Customer: {booking.customer_name}\n"
            f"Phone: {booking.phone}\n"
            f"Email: {booking.email}\n"
            f"Vehicle: {booking.vehicle}\n"
            f"Estimate ID: {booking.estimate_id}\n"
            f"Shop: {shop.name}\n"
    ...
    created = svc.events().insert(calendarId=shop.calendar_id, body=event).execute()
...
def create_calendar_event(shop: ShopConfig, booking: BookingRequest) -> str:
    ...
    print(
        f"[CALENDAR] Created event {event_id} for shop={shop.id}, "
...
# ============================================================
# MISSISSAUGA COLLISION CENTER WEB APP – /quote
# ============================================================
...
async def quote_page():
    ...
    <head>
      ...
      <title>Mississauga Collision Center – AI Damage Estimator</title>
      <style>
        :root{
          --sq-bg:#020617;
          --sq-card:#02091a;
            ...
            <header class="sq-header">
              <div class="sq-header-left">
                ...
                <div class="sq-header-text">
                  <div class="sq-header-title">MISSISSAUGA COLLISION CENTER</div>
