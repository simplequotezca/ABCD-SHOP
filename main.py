...
def apply_rule_overrides(ai: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for d in damage_items:
        ...
        # Normalize component naming to match UI expectations
    ...
    # Final conservative clamp: Severe must have at least some baseline hours implied
    # (We clamp later after computing hours.)
    return ai
...
def format_damage_bullets(damage_items: List[Dict[str, Any]]) -> List[str]:
    """
    Convert damage items into user-facing bullets like:
    - Driver fender
    - Passenger headlight
    - Front bumper
    """
    ...
    for d in damage_items:
        comp = d.get("component", "Unknown")
        side = d.get("side_driver_pov", "Unknown")
        # If component already includes "Front bumper" etc keep; else prefix side
...
async def ai_vision_analyze(photos: List[UploadFile]) -> Dict[str, Any]:
    ...
    """
    # Read and base64 encode up to 3 photos
    files = photos[:3]
    images_payload = []
    for f in files:
        data = await f.read()
        ...
        # Assume jpeg; works fine for png too as "image/*" in practice
        images_payload.append({
            "type": "image_url",
            "image_url": {"url": f"data:image/jpeg;base64,{b64}"},
        })

    # Conservative fallback if missing key / no images
    ...
    try:
        # Use chat.completions with response_format json_schema (works on modern OpenAI API)
        ...
        # Enforce presence of required keys (defensive)
        ...
        # Hard fallback (still conservative, still valid schema)
        return {
            "overall": {
                "severity": "Moderate",
                "confidence": "Low",
...
def render_upload(shop_key: str, shop_name: str) -> str:
    ...
    # Restores loading bar + prevents double submits.
    return f"""<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8" />
...
async def estimate_api(
    ...
    # If confidence low -> widen range a bit
    if confidence == "Low":
